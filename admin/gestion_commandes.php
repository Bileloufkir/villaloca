...
if ( !empty($_POST) ){
    ...
    if ($resultat->rowCount() > 0){
        ...
        if (  $etatactuel != $_POST['newetat'] ){
            ...
            $content .= '<div class="alert alert-success">La commande '.$_POST['id_commande'] .' est passée de l\'état <strong>'.$etatactuel.'</strong> à l\'état <strong>'.$_POST['newetat'].'</strong></div>';
        }
    }
}
...
if ( $commandes->rowCount() > 0){
    ...
    while( $cmd = $commandes->fetch() ){
         ...
         if ($oldusr != $cmd['idmembre']){
            ...
            <th colspan="3"><i class="far fa-address-card"></i> '.$cmd['adresse'].' '.$cmd['code_postale'].' '.$cmd['ville'].'</th>
        ...
        switch($cmd['etat']){
            ...
            case 'envoyé' : $class="bg-secondary text-light"; break;
            case 'livré' : $class="bg-success"; break;
        }
        ...
        if ($oldcmd != $cmd['nbcmd']){
            ...
            <label for="etat">Etat</label> 
                <select name="newetat" class="form-control mx-3 ">
                    ...
                    <option '.( ($cmd['etat'] == 'envoyé') ? 'selected':'' ).'>envoyé</option>
                    <option '.( ($cmd['etat'] == 'livré') ? 'selected':'' ).'>livré</option>
                </select>
        ...
        <td> '.$cmd['categorie'].'</td>
        <td><img src="'.URL . 'photo/' . $cmd['photo'].'" alt="" class="vignettecommande w-25"></td>
        <td>'. $cmd['prix'] .'€</td>
        <td> ' . $cmd['quantite'] . '</td>
    ...
    $content .= '<div class="alert alert-info">Il n\'y a pas encore de commandes</div>';
}
